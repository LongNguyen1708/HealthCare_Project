...
class PIIScanner:
    ...
    def scan_test(self, text:str) -> Dict[str,List[str]]:
        finding = {}
        for pii_type , information in self.information.items():
            match = re.findall(information, str(text) , re.IGNORECASE)
            if match:
                finding[pii_type] = match
        return finding
    def scan_dataframe(self,df:pd.DataFrame) -> Dict[str,List[str]]:
        ...
        for column in df.columns:
            ...
            for idx , value in df[column].items:
                pii_found  = self.scan_test(value)
                if pii_found:
                    column_findings.append({
                        'row' : idx,
                        'pii_type' : list(pii_found.keys())
                    })
            if column_findings:
